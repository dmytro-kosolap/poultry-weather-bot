...
async def get_usd_uah_rate(session: aiohttp.ClientSession, api_key: str) -> float | None:
    try:
...
async def get_usd_uah_rate_fallback(session: aiohttp.ClientSession) -> float | None:
    try:
...
async def get_grain_context():
                ...
                async with session.get(url, params=params, timeout=15) as response:
                    if response.status == 200:
                        ...
                        if "data" in data and data["data"]:
                            # value = долари за тонну (API вже конвертував)
                            ...
                            results.append(
                                f"{item['name']}: ~${price_usd:.0f}/т  "
                                f"<b>{price_uah:,.0f} грн/т</b>  {change_text}"
                            )
                        else:
    ...
    if results and not all(
        any(k in r for k in ("недоступ", "помилка", "сервіс")) for r in results
    ):
