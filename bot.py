...
async def get_weather_forecast():
    ...
    # ПОРАДИ ВІД GEMINI (жорсткий промпт від Gemini)
    try:
        prompt = (
            f"Ти лаконічний український фермер. Погода: {summary}. "
            "Напиши ОДНУ практичну пораду до 150 символів. "
            "ТІЛЬКИ ТЕКСТ ПОРАДИ. Без заголовків, без привітань, без лапок, без Markdown."
        )
        ...
        clean_advice = resp.text.strip().replace("*", "").replace("#", "").replace("_", "")
        
        # Обрізаємо до 200 символів
        text = clean_advice[:200]
