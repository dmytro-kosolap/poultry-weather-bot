...
async def get_poultry_advice(summary):
    ...
    prompt = (
        f"Ти експерт-птахівник. На завтра {tomorrow_date} прогноз: {summary}. "
        f"Напиши ґрунтовну пораду (до 700 символів). Згадай дату. "
        f"Акцент на нічних заморозках, воді, підстилці та калорійності корму. "
        f"Стиль: професійний, як у твоєму прикладі."
    )
    try:
        response = model.generate_content(prompt)
        return response.text.strip()
    except:
        return "Подбайте про тепло та енергію для птахів."
